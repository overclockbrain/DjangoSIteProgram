...
class MYAI {
    constructor(color, state) {
        ...
        this.optimalList.push({ count: -99999 });
    ...
    predict() {
        ...
        for (let node1_branch in node1_reverse) {
            ...
            // Pruning
            if (this.Pruning(node2_board, node1_reverse[node1_branch])) {
                break;
            }
            ...
            for (let node2_branch in node2_reverse) {
                ...
                // Pruning
                if (node2_branch == 0) {
                    for (let b in node2_reverse) {
                        if (this.Pruning(node3_board, node2_reverse[b])) {
                            console.log('isBad');
                            this.isBadMove = true;
                            break;
                        }
                ...
                if (this.isBadMove && this.optimalList[0]['count'] > 0) {
                    break;
                }
                this.Compare(
                    node2_board, node1_reverse[node1_branch], node2_reverse[node2_branch]
                );
    ...
    Pruning(board, state) {
        ...
        // console.log(PUT_STATE(state));
        return this.Corner(board, PUT_STATE(state)[0], PUT_STATE(state[1]));
    }
     ...
     * @returns 
     */
    CountKoma(board, isBlank = false) {
        const COLOR = (isBlank) ? 0 : this.CURRENT_COLER;
        let count = 0;
    ...
    Compare(board, node1 = [], node2 = [], node3 = [], node4 = [], node5 = []) {
        ...
        let remainState = this.CountKoma(board, true);

        latestCount += (this.isBadMove) ? 0 : 500;
        latestCount = (this.Corner(board, PUT_STATE(node1)[0], PUT_STATE(node1[1]))) ? 999 : latestCount;

        if (latestCount > bestCount) {
            this.optimalList = new Array();
            this.optimalList.push({
                count: latestCount, isBadMove: this.isBadMove,
